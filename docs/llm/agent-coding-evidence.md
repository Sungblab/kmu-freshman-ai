...
LLM이 생성한 코드를 그대로 제출하지 않기 위해 다음 기준으로 직접 검토했다.
...
따라서 발표에서는 “LLM이 준 코드를 붙여넣었다”가 아니라, “LLM을 초안 작성과 오류 분석에 사용했고, 최종 제출 전 Python 로직을 함수/조건문/반복문/테스트/주석/문서로 다시 정리했다”고 설명한다.

## 5. 과제 조건과 연결되는 에이전트 활용

| 과제 조건 | 에이전트 활용 방식 | 사람이 확인한 증거 |
| --- | --- | --- |
| Python 사용 | Codex가 FastAPI service/schema/test 초안을 만들고 수정 | `backend/app/`, `backend/tests/`, `pnpm test:backend` |
...
```txt
본 프로젝트는 Codex 같은 코딩 에이전트를 단순 코드 생성기가 아니라 개발 하네스 안에서 동작하는 보조 개발자로 사용했다. AGENTS.md, 문서 라우터, feature registry, Superpowers spec/plan, 전용 Codex 스킬로 작업 범위를 먼저 고정했고, 구현 후에는 테스트와 빌드, 문서 검증을 실행했다. 생성된 코드는 그대로 제출하지 않고 Python 함수 구조, 조건문/반복문, 추천 규칙, RAG 근거 처리, 일정 계산 로직을 직접 읽고 수정해 발표에서 설명 가능한 형태로 정리했다.
```
