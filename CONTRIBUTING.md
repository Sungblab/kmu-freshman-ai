...
## 코드 리뷰 대응
- Gemini Code Assist 또는 팀원이 남긴 주요 리뷰 코멘트
- 반영한 코멘트와 반영하지 않은 코멘트의 이유
- 설명을 위해 추가한 코드 주석 또는 문서 위치
```

## 6. 코드 리뷰와 주석 규칙

코드 리뷰와 주석은 “직접 이해하고 수정했다”는 증거입니다.

- 기능 PR은 가능하면 GitHub PR로 올려 Gemini Code Assist 자동 리뷰를 받습니다.
- 자동 리뷰가 맞는 지적이면 코드나 테스트로 반영하고, PR 대화에 반영 내용을 남깁니다.
- 자동 리뷰가 과하거나 틀렸으면 무시하지 말고 “왜 반영하지 않는지”를 짧게 답합니다.
- 복잡한 함수에는 발표자가 설명할 수 있도록 짧은 주석을 답니다.
- 당연한 코드에는 주석을 달지 않고, 의도/제약/예외 처리 이유를 설명하는 주석만 남깁니다.
- 중요한 리뷰 대응은 `docs/llm/usage-log.md` 또는 관련 plan 문서에도 요약합니다.

예시:

```py
# wiki page를 raw document보다 먼저 검색해서 넓은 질문의 검색 비용을 줄인다.
```
